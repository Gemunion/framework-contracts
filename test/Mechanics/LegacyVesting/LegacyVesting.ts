...
describe("LegacyVesting", function () {
  const factory = () => deployVesting("LegacyVesting", 12, 417);
  ...
  describe("release", function () {
    it("AdvisorsVesting", async function () {
      await calc("LegacyVesting", 12, 417);
    });

    it("MarketingVesting", async function () {
      await calc("LegacyVesting", 1, 1500);
    });

    it("PartnershipVesting", async function () {
      await calc("LegacyVesting", 6, 417);
    });

    it("PreSeedSaleVesting", async function () {
      await calc("LegacyVesting", 3, 416);
    });

    it("PrivateSaleVesting", async function () {
      await calc("LegacyVesting", 1, 624);
    });

    it("PublicSaleVesting", async function () {
      await calc("LegacyVesting", 0, 3333);
    });

    it("SeedSaleVesting", async function () {
      await calc("LegacyVesting", 2, 500);
    });
    ...
    it("InitialLiquidityVesting", async function () {
      await calc("LegacyVesting", 0, 5000);
    });

    it("TreasuryVesting", async function () {
      await calc("LegacyVesting", 3, 10000);
    });
  });
